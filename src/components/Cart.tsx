...
export default function Cart() {
  const { producia, setProducia, isEmpty } = useContext(MyContext);

  function incrementProductQuantity(id: number) {
    const updatedProducts = producia.map(product =>
      product.id === id ? { ...product, quantity: product.quanatty + 1 } : product
    );
  ...
  function decrementProductQuantity(id: number) {
    const updatedProducts = producia.map(product =>
      product.id === id && product.quanatty > 0 ? { ...product, quanatty: product.quanatty - 1 } : product
    );
    setProducia(updatedProducts.filter(product => product.quanatty > 0));
  }

  // Calculate total price
  const totalPrice = producia.reduce((acc, product) => acc + (product.price * product.quanatty), 0);
                ...
                <div className="">
                  {producia.length === 0 ? (
                    ...
                    producia.map((item: prodType, index) => (
                      <div className="flex flex-row justify-between items-center px-6" key={index}>
                        <img className="h-[64px] w-[64px] rounded-[8px] bg-[#F1F1F1]" src={item.img1} alt="" />
                        ...
                        <div className="bg-[#F1F1F1] flex justify-center items-center">
                          <button onClick={() => decrementProductQuantity(item.id)} className="w-[16px] opacity-25 mr-[21px]">
                            -
                          </button>
                          ...
                          <button onClick={() => incrementProductQuantity(item.id)} className="w-[16px] opacity-25 ml-[21px] mr-[21px]">
                            +
                          </button>
                        </div>
                      </div>
